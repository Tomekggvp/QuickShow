...
import { toggleFavorite } from '../action/favoriteActions'; 
  ...
  return show ? (
    <div className={`px-6 md:px-16 lg:px-40 pt-30 md:pt-50 ${isDark ? 'text-white' : 'text-black'}`}>
      ...
      <div className='flex flex-col md:flex-row gap-8 max-w-6x1 mx-auto'>
        <img src={show.movie.poster_path} alt="" className='max-md:mx-auto rounded-xl h-104 max-w-70 object-cover shadow-lg' />
        ...
        <div className='relative flex flex-col gap-3'>
          ...
          <p className={`${isDark ? 'text-gray-400' : 'text-gray-700'} mt-2 text-sm leading-tight max-w-xl`}>
            {show.movie.overview}
          </p>
          ...
          <div className='flex items-center flex-wrap gap-4 mt-4'>
            ...
            <a href="#dateSelect" className='px-10 py-3 text-sm bg-primary hover:bg-red-400 transition rounded-md font-medium cursor-pointer active:scale-95 text-white'>
              Buy Tickets
            </a>
            
            <button 
              onClick={() => dispatch(toggleFavorite(id))}
              className={`p-2.5 rounded-full transition-all cursor-pointer active:scale-95 border-none outline-none ${isLiked ? 'bg-red-500/20' : (isDark ? 'bg-gray-700' : 'bg-[#afceec3a]')}`}
            >
              <Heart 
      ...
      <div className='flex justify-center mt-20 mb-20'>
        <button 
          ...
          className='px-10 py-3 text-sm bg-red-400 hover:bg-red-300 transition rounded-md font font-medium cursor-pointer text-white'
        >
          Show more
        </button>
      </div>
