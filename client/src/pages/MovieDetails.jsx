  ...
  return show ? (
    <div className={`px-6 md:px-16 lg:px-40 pt-30 md:pt-50 ${isDark ? 'text-white' : 'text-black'}`}>
        ...
        <div className='relative flex flex-col gap-3 '>
          ...
          <div className='flex items-center flex-wrap gap-4 mt-4'>
           <button className='flex items-center gap-2 px-7 py-3 text-sm bg-gray-800 hover:bg-gray-900 transition rounded-md font-medium cursor-pointer active:scale-95 text-white'>
              ...
              <a href="#dateSelect" className='px-10 py-3 text-sm bg-primary hover:bg-red-400 transition rounded-md font-medium cursor-pointer active:scale-95'>Buy Tickets</a>
            
            <button 
              onClick={() => dispatch(toggleFavorite(id))}
              className={`bg-gray-700 p-2.5 rounded-full transition cursor-pointer active:scale-95 text-white ${isLiked ? 'bg-red-500/20' : (isDark ? 'bg-gray-700' : 'bg-[#afceec3a]')}`}
            >
              <Heart 
      ...
      <div className='flex justify-center mt-20 mb-20'>
        <button onClick={() => { navigate('/movies'); window.scrollTo(0, 0) }} className='px-10 py-3 text-sm bg-red-400 hover:bg-red-300 transition rounded-md font font-medium cursor-pointer text-white'>
          Show more
        </button>
      </div>
